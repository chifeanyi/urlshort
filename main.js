...
Check
for url typingfunction validURL(str) {
  var pattern = new RegExp('^(https?:\\/\\/)?' + protocol '((([a-z\\d]([a-z\\d-]*[a-z\\d])*)\\.)+[a-z]{2,}|' + domain name '((\\d{1,3}\\.){3}\\d{1,3}))' + OR ip(v4) address '(\\:\\d+)?(\\/[-a-z\\d%_.~+]*)*' + port and path '(\\?[;&a-z\\d%_.~+=-]*)?' + query string '(\\#[-a-z\\d_]*)?$', 'i');
  fragment locator
  return !!pattern.test(str);
}
POST method implementation: async function postData(url = '', data = {}) {
  ...
  if (urlin === "") {
    ...
    postData('https://rel.ink/api/links/', {
      ...
      $("#btnCopy").on("click", function() {
        ...
        Copy text to clipboard using chrome API $("#btnCopy").addClass("copy");
...
Empty text Errorfunction callError() {
  if (empty) {
